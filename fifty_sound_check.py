kana_d = {
    ...
    'shi (xi)': ['し', 'シ', '用(shi)吸管(し)喝水(シ)', '用吸管喝水'],
    ...
    'chi (qi)': ['ち', 'チ', '她是有(chi)七(ち)千(チ)万的土豪', '她是有七千万的土豪'],
    'tsu (ci)': ['つ', 'ツ', '小(tsu)刺(つ)(ツ)猬很可爱', '小刺猬很可爱'],
...
def logging(record):
    with open("./resource/fifty_practice_record.txt", "a") as f:
        f.write(f"[{time.ctime()}]: {record}\n")

def report(this_record, columns):
    records = []
    idx = "Correct Ratio: "
    with open("./resource/fifty_practice_record.txt", "r") as f:
    ...
    records.sort()
    print(f"Highest ratio: {records[-1][-1]}".center(columns))
